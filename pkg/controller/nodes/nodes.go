...
func (c *NodeInformer) Register(sharedInformerFactory informers.SharedInformerFactory) cache.SharedIndexInformer {
	...
	nodeInformer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			c.log.Debugf("added node %q to architecture map", obj.(*corev1.Node).Name)
		},
		UpdateFunc: func(old, new interface{}) {
			...
			c.log.Debugf("updated node %q in architecture map", old.(*corev1.Node).Name)
		},
		DeleteFunc: func(obj interface{}) {
			// remove node from the map
			c.nodes.Delete((obj.(*corev1.Node)).Name)
			c.log.Debugf("removed node %q from architecture map", obj.(*corev1.Node).Name)
		},
	})
